...
- 正式运行会生成本地邮件草稿 HTML/EML，并在配置收件人后生成 Gmail 草稿请求；Codex automation `ai-infra-daily-gmail-draft-handoff` 负责把 request 创建为 Gmail 草稿；自动发送默认关闭，见 `docs/DELIVERY.md`
...
- Gmail MCP 已验证 labels/drafts/test draft；2026-06-15 已新增每日 07:45 Codex automation 负责 request → Gmail draft；自动发送需另行确认
...
**状态：** Phase 2 ✅ MVP 已收口；reviewed Gmail 草稿/发送闭环已验证；reviewed Calendar 创建闭环已验证；本地 task-log 已接入；本地仍不直接执行账号动作
...
- 真实 Calendar 创建闭环已验证：reviewed Tool Agent request → MCP `create_event` → Calendar 读取/搜索验证可见
